...
public_download_file=/etc/public-download.sh
public_file_Check=$(cat ${public_download_file} 2>/dev/null)
if [ ! -f $public_download_file ] || [ -z "${public_file_Check}" ];then
	wget -O $public_download_file https://www.ecoo.top/update/soft_init/public-download.sh -T 30;
fi
. $public_download_file

if [ -z "${dl_url}" ];then
	download_url="https://www.ecoo.top"
else
	download_url=$dl_url
fi
...
judgment_parameters() {
  while [[ "$#" -gt '0' ]]; do
    case "$1" in
      ...
      '-l' | '--local')
        LOCAL_INSTALL='1'
        LOCAL_FILE="${2:?error: 请指定本地文件.}"
        break
        ;;
      '-p' | '--proxy')
        if [[ -z "${2:?error: 请指定代理地址.}" ]]; then
          exit 1
        fi
        PROXY="$2"
        shift
...
install_software() {
  ...
  if ${PACKAGE_MANAGEMENT_INSTALL} "$package_name"; then
    echo "info: $package_name 已安装."
  else
    echo "error: 安装包 of $package_name 失败, 请检查网络."
    exit 1
  fi
}
...
download_v2ray() {
  DOWNLOAD_LINK="$download_url/update/soft_init/v2ray/$RELEASE_VERSION/v2ray-linux-$MACHINE.zip"
...
show_help() {
  ...
  echo '  --version       Install the specified version of V2Ray, e.g., --version v5.6.0'
...
main() {
  ...
  display_alert "..." "V2Ray $RELEASE_VERSION 已安装." "info"
  display_alert "..." "必须自行配置可用节点才能正常使用." "info"
  display_alert "..." "节点配置文件在/usr/local/etc/v2ray/config.json." "info"
  display_alert "..." "群内讨论一切相关话题均涉嫌违规，建议加TG电报群" "info"
  display_alert "..." "电报群号: t.me/histb_nas" "info"
